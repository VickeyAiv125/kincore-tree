...
import { User, Calendar, Briefcase, Eye, MapPin, X, ArrowLeft, Plus, Heart, Map, Mail } from 'lucide-react';
...
const AddParents = () => {
    ...
    const handleSave = async () => {
        if (!target_person_id) {
            alert('No child selected. Please go back to the tree and select a member.');
            return;
        }
        ...
        try {
            ...
            const res = await fetch(`${API}/clantree/add-parent`, {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="max-w-4xl mx-auto text-left">
            <header className="mb-10 flex items-center justify-between">
                <div>
                    <h2 className="text-brand-orange text-[10px] font-black uppercase tracking-widest mb-1">Add Ancestor to {target_name || 'Member'}</h2>
                    <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-brand-darkText leading-tight">Add Parent</h1>
                </div>
                    ...
                    <div className="flex space-x-4">
                        {['Male', 'Female'].map((g) => (
                            <button
                                ...
                                className={`px-12 py-4 rounded-2xl text-xs font-black transition-all ${formData.gender === g ? 'bg-brand-orange text-white' : 'bg-gray-50 dark:bg-brand-darkBg text-gray-400 hover:bg-orange-50 dark:hover:bg-brand-orange/10 hover:text-brand-orange'}`}
                            >
                                {g === 'Male' ? 'Father' : 'Mother'}
                            </button>
                        ))}
                    </div>
                </div>
            ...
            <FormSection title="History & Location">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    <PremiumInput label="Anniversary Date" name="anniversary_date" icon={Heart} placeholder="MM/DD/YYYY" value={formData.anniversary_date} onChange={handleChange} />
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-12 mb-20 justify-end">
                ...
                >
                    {loading ? 'Adding...' : 'Save & Add Parent'}
                </button>
            </div>
        </div>
    );
