...
const EditEvent = () => {
    ...
    useEffect(() => {
        const fetchEvent = async () => {
            try {
                ...
                const response = await fetch(`${baseUrl}/families/events/${id}?t=${Date.now()}`, {
                ...
                if (response.ok) {
                    ...
                    setPreviewUrl(data.cover_photo_url || data.cover_image || null);

                    // Pre-select RSVP'd members
                    if (data.rsvps) {
                        setSelectedMembers(data.rsvps.map(r => r.user_id));
                    }

                    // Fetch members for THIS family space specifically
                    if (data.family_space_id) {
                        fetchMembers(data.family_space_id);
                    }
                }
            } catch (err) {
    ...
    const fetchMembers = async (familyId) => {
        try {
            ...
            const targetFamilyId = familyId || user?.family_id;

            // Use the families members endpoint to get BOTH account users and lineage persons
            const endpoint = `${baseUrl}/families/${targetFamilyId}/members?t=${Date.now()}`;
    ...
    const handleUpdate = async () => {
        if (!formData.title || !formData.startDate) {
            setNotification({ message: 'Please fill in required fields (Title, Start Date)', type: 'error' });
            return;
        }
        ...
        try {
            const baseUrl = import.meta.env.VITE_API_BASE_URL || `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api'}`;
            const user = JSON.parse(localStorage.getItem('user'));
            const endpoint = `${baseUrl}/families/events/${id}`;
            ...
            if (response.ok) {
                ...
                setNotification({ message: 'Error: ' + (err.error || 'Failed to update event'), type: 'error' });
            }
        } catch (err) {
        ...
        <div className="max-w-4xl mx-auto text-left py-4 animate-in fade-in duration-500">
            <header className="flex justify-between items-center mb-10">
                <div className="space-y-1">
                    <h1 className="text-3xl sm:text-4xl font-black text-gray-900 dark:text-brand-darkText tracking-tight">Edit Event</h1>
                    <p className="text-[10px] font-bold text-brand-orange uppercase tracking-widest leading-none opacity-80">Update your family gathering details</p>
                </div>
            ...
            >
                ...
                <div className="relative z-10 bg-white/40 dark:bg-brand-darkCard/40 backdrop-blur-xl p-6 rounded-3xl border border-white/20">
                    <h3 className="text-xl font-black text-gray-900 dark:text-brand-darkText mb-2">{previewUrl ? 'Change Event Photo' : 'Add Event Cover Photo'}</h3>
                    <p className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-8">High Quality PNG or JPG supported</p>
                    <input
                        type="file"
                        ref={fileInputRef}
                        className="hidden"
