...
const AddSpouse = () => {
    ...
    const handleSave = async () => {
        if (!target_person_id) {
            alert('No partner selected. Please go back to the tree and select a member.');
            return;
        }
    ...
    return (
        <div className="max-w-4xl mx-auto text-left">
            <header className="mb-10 flex items-center justify-between">
                <div>
                    <h2 className="text-brand-orange text-[10px] font-black uppercase tracking-widest mb-1">Add Life Partner to {target_name || 'Member'}</h2>
                    <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-brand-darkText leading-tight">Add Spouse</h1>
                </div>
            ...
            <FormSection title="Biography & Notes">
                <PremiumInput label="Spouse Bio" name="bio_notes" icon={BookOpen} placeholder="Brief history of the spouse..." type="textarea" value={formData.bio_notes} onChange={handleChange} />
            </FormSection>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-12 mb-20 justify-end">
                ...
                >
                    {loading ? 'Adding...' : 'Save Spouse Details'}
                </button>
            </div>
        </div>
    );
